...
class FilmMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./film.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./film.policy').acl);
  }


  async default (req, res, next) {

    console.log('Film middleware;')
