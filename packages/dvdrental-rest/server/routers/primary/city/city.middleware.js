...
class CityMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./city.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./city.policy').acl);
  }


  async default (req, res, next) {

    console.log('City middleware;')
